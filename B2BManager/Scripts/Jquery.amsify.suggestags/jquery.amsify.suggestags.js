...
THIS LIBRAIRY HAS BEEN CUSTOMISED FOR ELUX BY B2B SUPPORT
PLEASE DO NOT UPDATE IT. SOME FEATURE WILL DISAPPEAR.
       ...
       /**
        ...
        createHTML : function() {
            ...
            if (this.settings.disabled) {
                sTagsInput = '<input type="text" class="' + this.classes.sTagsInput.substring(1) + '"disabled="true"/>';
            } else {
                sTagsInput = '<input type="text" class="' + this.classes.sTagsInput.substring(1) + '" placeholder="' + this.defaultLabel + '">';
            }
          ...
          $(this.selectors.sTagsInput).keyup(function(e){
            var keycode = (e.keyCode ? e.keyCode : e.which);
              if (keycode == '13' || keycode == '188') {
                  var value = "";
                  if ($("li.active:first").length > 0) {
                      //We get the value of the active li because dataVal is different of val in input now.
                      var value = $.trim($("li.active:first").data('val').replace(/,/g, ''));
        ...
        setPageEvents: function () {
            var _self = this;
            //We add event on all page. If user has suggestions open and click outside the target/container, it's hide the suggestions.
            $(document).mouseup(function (e) {
                var container = $(_self.selectors.sTagsArea);
          ...
          if(this.settings.selectOnHover) {
            $(this.selectors.listArea).find(this.classes.listItem).hover(function(){
              ...
              $(this).addClass('active');
                $(_self.selectors.sTagsInput).val($(this).data('val').split("_")[0]);
            }, function() {
        ...
        processAjaxSuggestion : function(value, keycode) {
          var _self           = this;
            ...
            if (this.settings.suggestionsAction.AdditionnalParam) {
                for (var key in this.settings.suggestionsAction.AdditionnalParamList){
                    params[key] = $(this.settings.suggestionsAction.AdditionnalParamList[key]).val(); 
                }
            } 
          ...
          ajaxFormParams['success'] = function(data) {
            if(data && data.d) {
              //_self.settings.suggestions = $.extend(_self.settings.suggestions, data.d);
              _self.settings.suggestions = data.d;
                ...
                $(this).removeClass('active');
                  ...
                  if($item.length) {
                    ...
                    $item.addClass('active');
                      $(_self.selectors.sTagsInput).val($item.data('val').split("_")[0]);
                      var element = document.getElementsByClassName('active');
                      //we substract 16 because the div has 3px padding and 13px margin
                      $(_self.classes.list).scrollTop(element[0].offsetTop - 16);
                  }
                return false;
               }
          });
        ...
        suggestWhiteList : function(value, keycode) {
          ...
          if(found) {
            ...
            if($item.length == 1 && keycode != '8') {
                if ((this.settings.whiteList && this.isSimilarText(value.toLowerCase(), $item.data('val').toString().toLowerCase(), 40)) || this.isSimilarText(value.toLowerCase(), $item.data('val').toString().toLowerCase(), 60)) {
                $item.addClass('active');
        ...
        createList : function() {
          ...
          $.each(this.settings.suggestions, function(index, item){
              listHTML += '<li class="'+_self.classes.listItem.substring(1)+'" data-val="'+item+'">'+item.split("_")[0]+'</li>';
          });
          return listHTML;
        },
        ...
        addTag : function(value) {
          if(!value) return;
            var html = '<span class="' + this.classes.tagItem.substring(1) + '" data-val="' + value + '">' + value.split("_")[0]+' '+this.setIcon()+'</span>';
