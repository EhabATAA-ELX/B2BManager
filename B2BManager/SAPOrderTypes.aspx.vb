...
Partial Class SAPOrderTypes
    ...
    Private Sub RenderControls()
        ...
        If Not clsUser.Actions.Contains(ClsHelper.ActionDesignation.MAINTAIN_SAP_ORDER_TYPE_ADD_CHANGE_DELETE) Then
            gridSearch.MasterTableView.GetColumn("Actions").Visible = False
            gridSearch.MasterTableView.GetColumn("Actions").Display = False
            WindowSAPOrderType.Visible = False
            placeHolderSAPOrderManagement.Visible = False
            newSAPOrderType.Visible = False
        End If
    End Sub
    ...
    Public Property searchDs As DataTable
        Get
            ...
            If Session("OrderTypes") Is Nothing Then
                Session("OrderTypes") = ClsDataAccessHelper.FillDataTable("SAPOrderTypes.GetOrderTypes", GetSearchParameters(), CommandType.StoredProcedure)
            End If
            Return Session("OrderTypes")
        End Get
        Set(value As DataTable)
            Session("OrderTypes") = value
        End Set
    End Property
    ...
    Private Sub PopulateSearchGrid()
        ...
        End If
        ClsHelper.Log("Load SAP Order Types", ClsSessionHelper.LogonUser.GlobalID.ToString(), ClsHelper.ConvertSQLParametersToFriendlyText(GetSearchParameters()), watch.ElapsedMilliseconds, False, Nothing)
    End Sub
