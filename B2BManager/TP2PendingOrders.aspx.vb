...
Partial Class TP2PendingOrders
    ...
    Public Property searchDs As DataTable
        Get
            ...
            If Session("OrdersIntegratedInMaintenance") Is Nothing Then
                Session("OrdersIntegratedInMaintenance") = ClsDataAccessHelper.FillDataTable("Maintenance.GetOrdersIntegratedInMaintenance", GetSearchParameters(), CommandType.StoredProcedure)
            End If
            Return Session("OrdersIntegratedInMaintenance")
        End Get
        Set(value As DataTable)
            Session("OrdersIntegratedInMaintenance") = value
        End Set
    End Property
    ...
    Private Sub PopulateSearchGrid()
        ...
        End If
        ClsHelper.Log("Load TP2 Pending Orders ", ClsSessionHelper.LogonUser.GlobalID.ToString(), ClsHelper.ConvertSQLParametersToFriendlyText(GetSearchParameters()), watch.ElapsedMilliseconds, False, Nothing)
    End Sub
    Private Function GetSearchParameters() As List(Of SqlParameter)
        ...
        If (RadDateTimePickerFrom.SelectedDate IsNot Nothing) Then
            parameters.Add(New SqlParameter("From", RadDateTimePickerFrom.SelectedDate))
        End If
        If (RadDateTimePickerTo.SelectedDate IsNot Nothing) Then
            parameters.Add(New SqlParameter("To", RadDateTimePickerTo.SelectedDate))
        End If
        Return parameters
    End Function
