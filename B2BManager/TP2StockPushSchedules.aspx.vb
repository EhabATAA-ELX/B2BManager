...
Partial Class TP2StockPushSchedules
    ...
    Private Sub RenderControls()
        ...
        ClsHelper.RenderCountryDropDown(ddlCountry, selectedApplication.Countries, ClsSessionHelper.EbusinessSopID, True, True)
        If Not clsUser.Actions.Contains(ClsHelper.ActionDesignation.MAINTAIN_TP2_STOCK_PUSH_SCHEDULES_ADD_CHANGE_DELETE) Then
            gridSearch.MasterTableView.GetColumn("Actions").Visible = False
            gridSearch.MasterTableView.GetColumn("Actions").Display = False
            WindowScheduleSettings.Visible = False
            placeHolderManageScript.Visible = False
            newPushStockSetting.Visible = False
        End If
    End Sub
    ...
    Private Function GetSearchParameters() As List(Of SqlParameter)
        ...
        If ddlCountry.SelectedValue <> "All" Then
            parameters.Add(New SqlParameter("@CountrySopIDs", ddlCountry.SelectedValue))
        End If
        Return parameters
    End Function
    ...
    Public Property searchDs As DataTable
        Get
            ...
            If Session("scheduleSettings") Is Nothing Then
                Session("scheduleSettings") = ClsDataAccessHelper.FillDataTable("TP2StockPush.GetPushStockSettings", GetSearchParameters(), CommandType.StoredProcedure)
            End If
            Return Session("scheduleSettings")
        End Get
        Set(value As DataTable)
            Session("scheduleSettings") = value
        End Set
    End Property
    ...
    Private Sub PopulateSearchGrid()
        ...
        End If
        ClsHelper.Log("Load Stock Push Settings", ClsSessionHelper.LogonUser.GlobalID.ToString(), ClsHelper.ConvertSQLParametersToFriendlyText(GetSearchParameters()), watch.ElapsedMilliseconds, False, Nothing)
    End Sub
