...
Partial Class SpecificKeysManagement
    ...
    Protected Function GetActionRights() As String
        ...
        If ClsSessionHelper.LogonUser.Actions.Count > 0 Then
            ...
            script = String.Format(scriptTemplate,
                                   IIf(actions.Contains(ClsHelper.ActionDesignation.EDIT_SMS_LOCAL_VALUE), "true", "false"),
                                   IIf(actions.Contains(ClsHelper.ActionDesignation.EDIT_SMS_DEFAULT_AND_COMMENT_VALUES), "true", "false"),
                                   IIf(actions.Contains(ClsHelper.ActionDesignation.CHANGE_SMS_TYPE), "true", "false"),
                                   IIf(actions.Contains(ClsHelper.ActionDesignation.DELETE_SMS_KEY), "true", "false"),
                                   IIf(actions.Contains(ClsHelper.ActionDesignation.ADD_SMS_KEY), "true", "false"),
                                   IIf(actions.Contains(ClsHelper.ActionDesignation.DISPLAY_ALL_SMS_TYPES), "true", "false"))
        Else
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Else
            ...
            BindData()
            SmsKeyUpdatePanel.Update()
            ClsHelper.Log("Load Specific Management Keys", ClsSessionHelper.LogonUser.GlobalID.ToString(), Nothing, watch.ElapsedMilliseconds, False, Nothing)
        End If
    End Sub
    ...
    Private Sub RenderControls(Optional ByVal selectedValue As String = Nothing)
        ...
        Dim actions As List(Of ClsHelper.ActionDesignation) = ClsSessionHelper.LogonUser.Actions
        btnAddSmsKey.Visible = actions.Contains(ClsHelper.ActionDesignation.ADD_SMS_KEY)
    ...
    Private Sub RenderDropDownListType()
        ...
        If (actions.Contains(ClsHelper.ActionDesignation.DISPLAY_ALL_SMS_TYPES)) Then
            'If (True) Then
                ...
                For Each dc In ds.Tables(0).Columns
                    ...
                    If (dc.ColumnName.ToString <> "IsBoolean") Then
                        If (i = 0) Then
                            ...
                            If logonUser.Actions.Contains(ClsHelper.ActionDesignation.EDIT_SMS_DEFAULT_AND_COMMENT_VALUES) Or logonUser.Actions.Contains(ClsHelper.ActionDesignation.EDIT_SMS_LOCAL_VALUE) Then
                                ...
                                actionsHeader = "Edit"
                            End If
                            If logonUser.Actions.Contains(ClsHelper.ActionDesignation.DISPLAY_ALL_SMS_TYPES) Then
                                Dim img As New HtmlImage()
                            ...
                            If logonUser.Actions.Contains(ClsHelper.ActionDesignation.DELETE_SMS_KEY) Then
                                toolsCount += 1
                                actionsHeader = "Delete"
                            End If
                            If logonUser.Actions.Contains(ClsHelper.ActionDesignation.DELETE_SMS_KEY) And dr("CountryValue").ToString <> "" Then
                                Dim img As New HtmlImage()
    ...
    Public Shared Sub DeleteLine(EnvironmentID As String, SopId As String, KeyName As String)
        ...
        If user IsNot Nothing Then
            ...
            If ClsDataAccessHelper.ExecuteNonQuery("[Ebusiness].[SmsMgmt_DeleteCountryValue]", parameters) Then
                ClsHelper.LogEbusinessAction(EnvironmentID, SopId, "Delete Specific Management field", user.GlobalID, ClsHelper.ConvertSQLParametersToFriendlyText(parameters), watcher.ElapsedMilliseconds, False, Nothing)
            Else
                ClsHelper.LogEbusinessAction(EnvironmentID, SopId, "Delete Specific Management field", user.GlobalID, ClsHelper.ConvertSQLParametersToFriendlyText(parameters), watcher.ElapsedMilliseconds, True, "An unexpected error has occurred")
            End If
        End If
    End Sub
